...
import BatchCreator   from '../components/social/BatchCreator.jsx';
import ApprovalSender   from '../components/social/ApprovalSender.jsx';
import PostPublisher   from '../components/social/PostPublisher.jsx';
...
const STATUS_META = {
  ...
  pending_client_approval: { label: 'Sent to Client',   color: '#a78bfa' },
  client_approved:         { label: 'Client Approved',  color: T.grn  },
  changes_requested:       { label: 'Changes Needed',   color: T.red  },
  approved:                { label: 'Manager Approved', color: T.blu  },
...
function SocialInner({ user, showToast }) {
  ...
  const [approvalSenderOpen, setASOpen]     = useState(false);
  const [approvalPosts, setApprovalPosts]   = useState([]);
  const [publisherPost, setPublisherPost]   = useState(null);
  const [mobileView, setMobileView]         = useState('main'); // main | sidebar | ai
  ...
  // ── Helpers ──
  const totalPending = posts.filter(p =>
    ['pending_review','pending_client_approval','client_approved','changes_requested'].includes(p.status)
  ).length;
  ...
  // ── Post status updates ──
  function updatePostStatus(postId, status, extra = {}) {
    setPosts(prev => prev.map(p => p.id === postId ? { ...p, status, ...extra } : p));
    api('posts-update', { method: 'PATCH', body: JSON.stringify({ id: postId, status, ...extra }) }).catch(() => {});
  ...
  // ── Render ──
  // Mobile: collapse to single column, use bottom nav
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, display: 'flex', background: T.ink9, color: T.fg, fontFamily: SANS, overflow: 'hidden' }}>
      ...
      {/* ══ LEFT SIDEBAR ══════════════════════════════════════════════════ */}
      <aside style={{ width: mobile ? '100%' : 220, background: T.ink8, borderRight: mobile ? 'none' : `1px solid ${T.ink7}`, display: mobile && mobileView !== 'sidebar' ? 'none' : 'flex', flexDirection: 'column', flexShrink: 0 }}>
        <div style={{ padding: '16px 16px 12px', display: 'flex', alignItems: 'center', gap: 9, borderBottom: `1px solid ${T.ink7}` }}>
          ...
          {mobile && (
            <button onClick={() => setMobileView('main')} style={{ marginLeft:'auto', background:'none', border:'none', color:T.ink2, fontSize:22, cursor:'pointer', lineHeight:1, padding:'0 4px' }}>×</button>
          )}
        </div>
        ...
        <nav style={{ padding: '10px 10px 6px', display: 'flex', flexDirection: 'column', gap: 2 }}>
          {[
            ...
            <button key={id} onClick={() => { setView(id); if(mobile) setMobileView('main'); }} style={{
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '4px 6px 8px' }}>
          {clients.map(c => (
            <button key={c.id} onClick={() => { selectClient(c.id); if(mobile) setMobileView('main'); }} style={{
        ...
        <div style={{ padding: '10px 12px', borderTop: `1px solid ${T.ink7}`, display: 'flex', flexDirection: 'column', gap: 6 }}>
          <button onClick={() => setBatchOpen(true)} style={btn('primary', { width: '100%' })}>+ New Posts</button>
          <button onClick={() => openComposer()} style={btn('ghost', { width: '100%' })}>+ Single Post</button>
      ...
      {/* ══ MAIN ═══════════════════════════════════════════════════════════ */}
      <main style={{ flex: 1, minWidth: 0, overflowY: 'auto', background: T.ink9, display: mobile && mobileView !== 'main' ? 'none' : 'block',
        paddingBottom: mobile ? 70 : 0 /* room for bottom nav */ }}>
        ...
        {view === 'approvals' && <Approvals posts={posts} clients={clients} onUpdate={updatePostStatus} onEdit={(p) => openComposer(p)}
            onSendForApproval={(selectedPosts) => { setApprovalPosts(selectedPosts); setASOpen(true); }}
            onPublish={(p) => setPublisherPost(p)} />}
        {view === 'analytics' && <Analytics posts={posts} clients={clients} />}
      </main>

      {/* ══ BATCH CREATOR OVERLAY ════════════════════════════════════════ */}
      {batchOpen && (
        <div style={{ position: 'fixed', inset: 0, background: T.ink9, zIndex: 900, overflowY: 'auto' }}>
          <div style={{ padding: '16px 12px 8px', display: 'flex', alignItems: 'center', gap: 10, borderBottom: `1px solid ${T.ink7}` }}>
            <button onClick={() => setBatchOpen(false)} style={{ background: 'none', border: 'none', color: T.ink2, cursor: 'pointer', fontSize: 22, lineHeight: 1, padding: '0 4px' }}>←</button>
            <span style={{ fontFamily: SERIF, fontSize: 17, color: T.fg }}>Batch Post Creator</span>
            {selected && <span style={{ fontFamily: MONO, fontSize: 10, color: T.ink3, letterSpacing: '.06em' }}>{selected.name.toUpperCase()}</span>}
          </div>
          <BatchCreator
            client={selected}
            showToast={showToast}
            onSaved={(savedPosts) => {
              ...
              setBatchOpen(false);
              setView('approvals');
              showToast?.(`${savedPosts.length} draft${savedPosts.length !== 1 ? 's' : ''} saved`);
            }}
          />
        </div>
      )}

      {/* ══ APPROVAL SENDER MODAL ═════════════════════════════════════════ */}
      {approvalSenderOpen && (
        <ApprovalSender
          posts={approvalPosts}
          client={clients.find(c => c.id === approvalPosts[0]?.client_id)}
          showToast={showToast}
          onClose={() => setASOpen(false)}
          onSent={(token) => {
            setASOpen(false);
            setView('approvals');
            showToast?.('Posts sent to client for approval!', 'success');
          }}
        />
      )}

      {/* ══ POST PUBLISHER BOTTOM SHEET ═══════════════════════════════════ */}
      {publisherPost && (
        <PostPublisher
          post={publisherPost}
          client={clients.find(c => c.id === publisherPost.client_id)}
          onClose={() => setPublisherPost(null)}
          onMarkPosted={() => {
            updatePostStatus(publisherPost.id, 'posted');
            setPublisherPost(null);
            showToast?.('Post marked as live ✓', 'success');
          }}
        />
      )}
      ...
      {mobile && (
        <nav style={{
          ...
          background: T.ink8, borderTop: `1px solid ${T.ink7}`,
          display: 'flex', alignItems: 'stretch', height: 62,
          paddingBottom: 'env(safe-area-inset-bottom)',
        }}>
          {[
            { id: 'sidebar', icon: '☰', label: 'Menu' },
            { id: 'main',    icon: view === 'approvals' ? '◈' : view === 'calendar' ? '▤' : view === 'campaign' ? '✦' : '◇', label: view === 'approvals' ? 'Approvals' : view === 'calendar' ? 'Calendar' : view === 'campaign' ? 'Campaign' : 'Clients' },
            { id: 'main_approvals', icon: '◈', label: 'Approvals', badge: totalPending },
            { id: 'ai',      icon: '✦', label: 'AI' },
          ].filter(n => n.id !== 'main_approvals' || totalPending > 0 || view === 'approvals').map(nav => {
            // "main_approvals" is a shortcut that sets both mobileView and view
            const isActive = nav.id === 'main_approvals'
              ? (mobileView === 'main' && view === 'approvals')
              : nav.id === 'main'
                ? (mobileView === 'main' && view !== 'approvals')
                : mobileView === nav.id;
            return (
              <button key={nav.id} onClick={() => {
                if (nav.id === 'main_approvals') { setMobileView('main'); setView('approvals'); }
                else setMobileView(nav.id);
              }} style={{
                flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center',
                justifyContent: 'center', gap: 3, background: 'none', border: 'none',
                cursor: 'pointer', position: 'relative',
                color: isActive ? T.acc : T.ink3,
              }}>
                <span style={{ fontSize: 18, fontFamily: SERIF, lineHeight: 1 }}>{nav.icon}</span>
                <span style={{ fontFamily: MONO, fontSize: 9, letterSpacing: '.06em' }}>{nav.label}</span>
                {nav.badge > 0 && (
                  <span style={{
                    position: 'absolute', top: 8, right: '50%', marginRight: -18,
                    background: T.amb, color: T.ink9, fontSize: 8, fontFamily: MONO,
                    fontWeight: 700, padding: '1px 5px', borderRadius: 999,
                  }}>{nav.badge}</span>
                )}
                {isActive && (
                  <span style={{ position: 'absolute', top: 0, left: '20%', right: '20%', height: 2, background: T.acc, borderRadius: '0 0 2px 2px' }} />
                )}
              </button>
            );
          })}
      ...
      <aside style={{ width: mobile ? '100%' : 320, background: T.ink8, borderLeft: mobile ? 'none' : `1px solid ${T.ink7}`, display: mobile && mobileView !== 'ai' ? 'none' : 'flex', flexDirection: 'column', flexShrink: 0 }}>
...
// ════════════════════════════════════════════════════════════════════════════
// APPROVALS — full workflow hub
// Tabs: Draft → Send for Approval → Client Reviews → Schedule
// ════════════════════════════════════════════════════════════════════════════
function ScheduleModal({ post, client, onSave, onClose }) {
  const [dt, setDt] = useState(
    post.scheduled_at ? new Date(post.scheduled_at).toISOString().slice(0,16) : ''
  );
  return (
    <div onClick={e => { if (e.target === e.currentTarget) onClose(); }}
      style={{ position:'fixed', inset:0, background:'#00000088', display:'grid', placeItems:'center', zIndex:1000, padding:16 }}>
      <div style={{ background:T.ink8, border:`1px solid ${T.ink7}`, borderRadius:14, padding:24, width:'100%', maxWidth:360 }}>
        <div style={{ fontFamily:SERIF, fontSize:17, color:T.fg, marginBottom:4 }}>Schedule Post</div>
        {client && <div style={{ fontFamily:MONO, fontSize:10, color:T.ink3, letterSpacing:'.06em', marginBottom:14 }}>{client.name.toUpperCase()} · {post.platform?.toUpperCase()}</div>}
        <p style={{ fontSize:12, color:T.ink2, lineHeight:1.5, marginBottom:16 }}>
          {post.caption?.slice(0,80)}{post.caption?.length > 80 ? '…' : ''}
        </p>
        <label style={{ display:'block', marginBottom:16 }}>
          <div style={{ fontFamily:MONO, fontSize:10, color:T.ink3, letterSpacing:'.08em', marginBottom:6 }}>DATE & TIME</div>
          <input type="datetime-local" value={dt} onChange={e => setDt(e.target.value)}
            style={{ width:'100%', boxSizing:'border-box', padding:'9px 12px', background:T.ink9, border:`1px solid ${T.ink7}`, borderRadius:8, color:T.fg, fontFamily:SANS, fontSize:13, outline:'none' }} />
        </label>
        <div style={{ display:'flex', gap:8 }}>
          <button onClick={onClose} style={btn('ghost', { flex:1 })}>Cancel</button>
          <button onClick={() => dt && onSave(dt)} disabled={!dt} style={btn('success', { flex:2 })}>Save Schedule ✓</button>
        </div>
      </div>
    </div>
  );
...
function Approvals({ posts, clients, onUpdate, onEdit, onSendForApproval, onPublish }) {
  const [tab,       setTab]       = useState('drafts');
  const [selected,  setSelected]  = useState(new Set());
  const [schedPost, setSchedPost] = useState(null);
  const [clientFilter, setCF]     = useState('all');

  const byStatus = {
    drafts:           posts.filter(p => p.status === 'draft' || p.status === 'pending_review'),
    sent_to_client:   posts.filter(p => p.status === 'pending_client_approval'),
    client_approved:  posts.filter(p => p.status === 'client_approved'),
    changes_needed:   posts.filter(p => p.status === 'changes_requested'),
    scheduled:        posts.filter(p => p.status === 'scheduled' || p.status === 'approved'),
  };

  const activeList = (byStatus[tab] || [])
    .filter(p => clientFilter === 'all' || p.client_id === clientFilter)
    .sort((a, b) => new Date(a.scheduled_at || 0) - new Date(b.scheduled_at || 0));

  const tabs = [
    { id:'drafts',          label:'Drafts',           count: byStatus.drafts.length,          color: T.ink3 },
    { id:'sent_to_client',  label:'Sent to Client',   count: byStatus.sent_to_client.length,  color:'#a78bfa' },
    { id:'client_approved', label:'Client Approved',  count: byStatus.client_approved.length, color: T.grn },
    { id:'changes_needed',  label:'Changes Needed',   count: byStatus.changes_needed.length,  color: T.red },
    { id:'scheduled',       label:'Scheduled',        count: byStatus.scheduled.length,       color: T.pur },
  ];
  ...
  function toggleAll() {
    ...
    else setSelected(new Set(activeList.map(p => p.id)));
  }

  const selectedPosts = activeList.filter(p => selected.has(p.id));
  const canSendForApproval = tab === 'drafts' && selectedPosts.length > 0;

  return (
    <div style={{ padding: '28px 32px' }}>
      {/* Due-now alert banner */}
      {(() => {
        const due = posts.filter(p => (p.status === 'scheduled' || p.status === 'approved') && p.scheduled_at && new Date(p.scheduled_at).getTime() <= Date.now());
        if (!due.length) return null;
        return (
          <div style={{ marginBottom:16, padding:'14px 16px', background:'#f5a62322', border:'1.5px solid #f5a623', borderRadius:14, display:'flex', alignItems:'center', gap:12 }}>
            <span style={{ fontSize:22 }}>⏰</span>
            <div style={{ flex:1 }}>
              <div style={{ fontFamily:SANS, fontSize:14, fontWeight:600, color:'#f5a623' }}>
                {due.length} post{due.length !== 1 ? 's' : ''} due now!
              </div>
              <div style={{ fontFamily:SANS, fontSize:12, color:T.fg2, marginTop:2 }}>
                Go to Scheduled tab → tap Post ↑
              </div>
            </div>
            <button onClick={() => setTab('scheduled')} style={{ padding:'8px 14px', background:'#f5a623', border:'none', borderRadius:8, color:'#0e1014', fontFamily:SANS, fontSize:12, fontWeight:700, cursor:'pointer' }}>
              View →
            </button>
          </div>
        );
      ...
      <div style={{ display:'flex', alignItems:'baseline', gap:12, marginBottom:4, flexWrap:'wrap' }}>
        <h1 style={{ fontFamily:SERIF, fontWeight:400, fontSize:30, color:T.fg, margin:0, letterSpacing:'-.02em' }}>Approvals</h1>
        {canSendForApproval && (
          <button onClick={() => onSendForApproval(selectedPosts)} style={btn('primary')}>
            ✉ Send {selectedPosts.length} for Approval
          </button>
        )}
        {tab === 'client_approved' && selectedPosts.length > 0 && (
          <button onClick={() => setSchedPost(selectedPosts[0])} style={btn('success')}>
            ⏱ Schedule {selectedPosts.length > 1 ? `${selectedPosts.length} posts` : 'post'}
          </button>
        )}
      </div>
      <p style={{ fontSize:13, color:T.ink3, margin:'0 0 18px' }}>Manage post approvals and scheduling</p>

      {/* Status tabs */}
      <div style={{ display:'flex', gap:4, marginBottom:14, flexWrap:'wrap', borderBottom:`1px solid ${T.ink7}`, paddingBottom:8 }}>
        {tabs.map(t => (
          <button key={t.id} onClick={() => { setTab(t.id); setSelected(new Set()); }} style={{
            padding:'5px 12px', borderRadius:6, fontSize:12, fontFamily:SANS, cursor:'pointer',
            background: tab === t.id ? t.color + '22' : 'transparent',
            color: tab === t.id ? t.color : T.ink3,
            border: `1px solid ${tab === t.id ? t.color + '55' : 'transparent'}`,
          }}>
            {t.label}
            {t.count > 0 && <span style={{ marginLeft:5, fontFamily:MONO, fontSize:9, opacity:.8 }}>{t.count}</span>}
          </button>
        ))}
      </div>
      ...
      <div style={{ display:'flex', gap:5, marginBottom:16, flexWrap:'wrap' }}>
        <button onClick={() => setCF('all')} style={{ padding:'4px 10px', borderRadius:999, fontSize:10, fontFamily:MONO, cursor:'pointer', background: clientFilter==='all' ? T.acc : 'transparent', color: clientFilter==='all' ? T.ink9 : T.ink2, border:`1px solid ${clientFilter==='all' ? T.acc : T.ink5}` }}>All Clients</button>
        {clients.map(c => (
          <button key={c.id} onClick={() => setCF(c.id)} style={{ padding:'4px 10px', borderRadius:999, fontSize:10, fontFamily:MONO, cursor:'pointer', background: clientFilter===c.id ? c.color+'28' : 'transparent', color: clientFilter===c.id ? c.color : T.ink2, border:`1px solid ${clientFilter===c.id ? c.color : T.ink5}` }}>{c.name}</button>
        ))}
      </div>
      ...
      {activeList.length === 0 ? (
        <div style={{ background:T.ink8, border:`1px dashed ${T.ink7}`, borderRadius:10, padding:40, textAlign:'center', color:T.ink3, fontSize:13 }}>
          {tab === 'drafts' ? 'No drafts — create posts from the sidebar.' :
           tab === 'sent_to_client' ? 'No posts waiting on client.' :
           tab === 'client_approved' ? 'No client-approved posts yet.' :
           tab === 'changes_needed' ? 'No change requests.' :
           'No scheduled posts.'}
        </div>
      ) : (
        <>
          {(tab === 'drafts' || tab === 'client_approved') && (
            <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:10 }}>
              <label style={{ display:'flex', alignItems:'center', gap:6, cursor:'pointer', fontFamily:SANS, fontSize:12, color:T.ink2 }}>
                <input type="checkbox" checked={selected.size === activeList.length && activeList.length > 0} onChange={toggleAll}
                  style={{ accentColor:T.acc, width:14, height:14 }} />
                Select all ({activeList.length})
              </label>
              {selected.size > 0 && <span style={{ fontFamily:MONO, fontSize:10, color:T.acc }}>{selected.size} selected</span>}
            </div>
          )}
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:9 }}>
            {activeList.map(p => {
              ...
              const isSelectable = tab === 'drafts' || tab === 'client_approved';
              const isSel = selected.has(p.id);
              return (
                <div key={p.id} onClick={() => isSelectable && toggleSelect(p.id)}
                  style={{ display:'flex', alignItems:'center', gap:14, padding:14,
                    background: isSel ? T.ink7 : T.ink8,
                    border:`1px solid ${isSel ? T.acc+'66' : T.ink7}`, borderRadius:10,
                    cursor: isSelectable ? 'pointer' : 'default', transition:'all .1s' }}>
                  {isSelectable && (
                    <input type="checkbox" checked={isSel} onChange={() => toggleSelect(p.id)} onClick={e => e.stopPropagation()}
                      style={{ accentColor:T.acc, width:15, height:15, flexShrink:0 }} />
                  )}
                  <div style={{ width:48, height:48, borderRadius:8, background:(c?.color||T.ink7)+'40', display:'grid', placeItems:'center', color:c?.color||T.ink3, fontSize:18, flexShrink:0 }}>
                    {PLAT[p.platform]?.icon}
                  </div>
                  <div style={{ flex:1, minWidth:0 }}>
                    <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:4, flexWrap:'wrap' }}>
                      {c && <span style={{ width:16, height:16, borderRadius:4, background:c.color, color:'#fff', display:'grid', placeItems:'center', fontSize:7, fontWeight:700, fontFamily:MONO, flexShrink:0 }}>{c.initials}</span>}
                      ...
                      <span style={tag(sm.color||T.ink3)}>{sm.label||p.status}</span>
                      {p.scheduled_at && (
                        <span style={{ fontSize:10, color:T.ink3, fontFamily:MONO, marginLeft:'auto' }}>
                          {new Date(p.scheduled_at).toLocaleString('en-US', { month:'short', day:'numeric', hour:'numeric', minute:'2-digit' })}
                        </span>
                      )}
                      {p.client_approval_note && (
                        <span style={{ fontSize:10, color:T.red, fontFamily:SANS, fontStyle:'italic' }} title={p.client_approval_note}>
                          ✎ note
                        </span>
                      )}
                    </div>
                    <p style={{ margin:0, fontSize:12, color:T.ink2, lineHeight:1.5, display:'-webkit-box', WebkitLineClamp:2, WebkitBoxOrient:'vertical', overflow:'hidden' }}>
                      {p.caption || <em style={{ color:T.ink3 }}>No caption</em>}
                    </p>
                    {p.client_approval_note && tab === 'changes_needed' && (
                      <p style={{ margin:'6px 0 0', fontSize:11, color:T.red, fontStyle:'italic', lineHeight:1.4 }}>"{p.client_approval_note}"</p>
                    )}
                  </div>
                  <div style={{ display:'flex', gap:6, flexShrink:0, flexWrap:'wrap', justifyContent:'flex-end', maxWidth: 120 }} onClick={e => e.stopPropagation()}>
                    {(tab === 'drafts' || tab === 'changes_needed') && (
                      <button onClick={() => onEdit(p)} style={btn('ghost')}>Edit</button>
                    )}
                    {tab === 'client_approved' && (
                      <button onClick={() => setSchedPost(p)} style={btn('success')}>⏱ Schedule</button>
                    )}
                    {tab === 'scheduled' && (
                      <button onClick={() => onPublish?.(p)} style={{
                        ...btn('primary'),
                        background: '#d96b3a', border: 'none',
                        display: 'flex', alignItems: 'center', gap: 5,
                      }}>Post ↑</button>
                    )}
                    {tab === 'drafts' && (
                      <button onClick={() => onUpdate(p.id, 'draft')} style={btn('danger')}>Delete</button>
                    )}
                  </div>
                </div>
              );
      ...
      {schedPost && (
        <ScheduleModal
          post={schedPost}
          client={clients.find(c => c.id === schedPost.client_id)}
          onClose={() => setSchedPost(null)}
          onSave={async (dt) => {
            await onUpdate(schedPost.id, 'scheduled', { scheduled_at: new Date(dt).toISOString() });
            setSchedPost(null);
            setSelected(new Set());
          }}
        />
